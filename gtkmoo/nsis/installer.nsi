...
;  File "QtXml4.dll"
  File "gtkmoo.exe"
  File "@CMAKE_SOURCE_DIR@/share/gtkmoo.ico"
...
Section "Start Menu Shortcuts"
  ...
  CreateShortCut "$SMPROGRAMS\@PROJECT_NAME_LONG@\@PROJECT_NAME_SHORT@.lnk" "$INSTDIR\gtkmoo.exe" "" "$INSTDIR\gtkmoo.ico" 0
  CreateShortCut "$SMPROGRAMS\@PROJECT_NAME_LONG@\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
SectionEnd

Section "Desktop Shortcut"
  CreateShortCut "$DESKTOP\@PROJECT_NAME_LONG@.lnk" "$INSTDIR\gtkmoo.exe" "" "$INSTDIR\gtkmoo.ico" 0
SectionEnd
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\gtkmoo.exe"
  Delete "$INSTDIR\gtkmoo.ico"
