...
public class SaveCourseware extends HttpServlet
{
 @Override
 ...
 throws ServletException, IOException
 {
  ...
  while(true)
  {
    ...
    filename = header.substring(startName+10, endName);
    
        
          ServletContext sc = request.getSession().getServletContext();
  File file = new File(sc.getRealPath("/courseware"));
  
  file.mkdirs();
  FileOutputStream fos = new FileOutputStream(
          sc.getRealPath("/courseware")+"/"+filename);
  ...
  fos.close();
   ...
   try
   {
    Connection con = Utils.getDBConnection();
     PreparedStatement cs = con.prepareStatement("insert into courseware (uploader, filename) values (?,?)");
     cs.setString(1, userData.getUser());
 ...
 private String destination = "SubCourseware";
}
